...
content_id: 890785bd347228988f2cf89d2b65e7acf74759da554342b27df176b1a2596d67
---
...
Deno.test("PlannerConcept: Operational Principle", async () => {
  ...
  try {
    ...
    const scheduleResult = await planner._getScheduledTasks({ user });
    console.log(" > _getScheduledTasks result:", scheduleResult);

    // FIX: A query returns an array of results (frames). We expect one frame here.
    assert(scheduleResult.length === 1, "Query should return one result frame.");
    const schedule = scheduleResult[0]; // Unwrap the result from the array.
