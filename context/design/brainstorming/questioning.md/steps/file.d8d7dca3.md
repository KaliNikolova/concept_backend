...
content_id: d8d7dca38f2e708548eea40360fe9a4596d90f1e6febc72f9fb0d18699a7055d
---
...
export default class UserAccountConcept {
  ...
  /**
   * @query _getUserProfile (user: User): (profile: {displayName: String, email: String})
   * @effects returns the display name and email of a user.
   * @param {Object} args
   * @param {User} args.user - The ID of the user.
   * @returns {Promise<{profile: { displayName: string; email: string }}[]>} An array containing the user's profile or an empty array if not found.
   */
  async _getUserProfile(
    ...
    // CORRECT: Return an array containing the result, with the key 'profile'
    return [{
      profile: {
  ...
  /**
   * @query _findUserByEmail (email: String): (user: User)
   * @effects returns the user ID if an email exists.
   * @param {Object} args
   * @param {string} args.email - The email to search for.
   * @returns {Promise<{user: User}[]>} An array containing the user ID or an empty array if not found.
   */
  async _findUserByEmail(
    ...
    // CORRECT: Return an array containing the result, with the key 'user'
    return [{ user: userDoc._id }];
  }
}
```

With the concept now returning the correct shape, the `auth.sync.ts` file works as intended without any changes. Here it is again for completeness.
