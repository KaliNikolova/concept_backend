...
content_id: 4e8ff3ea3eac7dfda3bd26e5793e22a03fc2c5fe767e54dcf6b9dcf90ca940f2
---
...
Deno.test("PlannerConcept: Operational Principle", async () => {
  ...
  try {
    ...
    // FIX: The query returns an array, so we must unpack it first.
    const scheduleResult = await (planner as any)._getScheduledTasks({ user });
    console.log(" > _getScheduledTasks result:", scheduleResult);
    assertEquals(scheduleResult.length, 1, "A plan should exist for the user.");
    const schedule = scheduleResult[0]; // Get the plan object from the array
...
Deno.test("PlannerConcept: Interesting Scenarios", async (t) => {
  ...
  // This test does not depend on time, but we use the mock provider for consistency.
  await t.step("Scenario 3: Clearing and deleting plans", async () => {
    ...
    try {
      ...
      // Note: clearDay only clears for "today", so this will pass with our mock time.
